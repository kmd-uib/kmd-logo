...
const letterMap = {
    'K': K, 'U': U, 'N': N, 'S': S, 'T': T, 'M': M, 'I': I,
    'D': D, 'E': E, 'G': G, 'X': X, '▝': Block
};
...
const generateAnchors = (length) => {
    const anchors = [];
    for (let i = 0; i < length; i++) {
        anchors.push(i / (length - 1));
    }
    return anchors;
};
...
const Logo = ({
    ...
    } else if (text === 'KMDEXIT▝') {
        targetAnchors = kmdexitAnchors[mode] || generateAnchors(letters.length);
    } else {
        targetAnchors = generateAnchors(letters.length);
    }
    ...
    const updatePositions = useCallback(() => {
        animationRef.current = requestAnimationFrame(() => {
            const newValues = letters.map((_, index) => {
                ...
                // No special anchoring - use standard physics for all letters
    ...
    }, [positions, velocities, text, width, constants, targetAnchors, mode, letters]);
    ...
    }, [text, mode, width, constants, updatePositions]);
